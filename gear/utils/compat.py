...
def thor_receipt_convert_to_eth_receipt(receipt):
    return {
        ...
        "transactionHash": receipt["tx"]["id"],
        "transactionIndex": encode_number(0),
        "blockNumber": encode_number(receipt["block"]["number"]),
        "blockHash": receipt["block"]["id"],
...
def thor_tx_convert_to_eth_tx(tx):
    return {
        "hash": tx["id"],
        "nonce": tx["nonce"],
        "blockHash": tx["block"]["id"],
        "blockNumber": encode_number(tx["block"]["number"]),
        "transactionIndex": encode_number(0),
        "from": tx["origin"],
        "to": tx["clauses"][0]["to"],
